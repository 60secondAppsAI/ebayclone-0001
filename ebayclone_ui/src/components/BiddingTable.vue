        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalBiddings = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalBiddings">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Bidding</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="BiddingId" type="text" placeholder="Enter BiddingId" v-model="biddingToAdd.biddingId"></base-input>
  <base-input label="BidAmount" type="text" placeholder="Enter BidAmount" v-model="biddingToAdd.bidAmount"></base-input>
  <base-input label="BidDate" type="text" placeholder="Enter BidDate" v-model="biddingToAdd.bidDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="biddings" :row-key="record => record.BiddingId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <BiddingPictureView :biddings="biddings" />
          </div>
...
import BiddingPictureView from './BiddingPictureView.vue';


const biddingsColumns = [
  "biddingId",
  "year",
  "date",
  "competitionId",
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'biddingId',
          ...
          sorter: true
          //sorter: (a, b) => a.biddingId - b.biddingId,
          //sorter: (a, b) => a.biddingId.localeCompare(b.biddingId),
        },
        {
          title: 'Bid Amount',
		dataIndex: 'bidAmount',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} biddings`,
      },
      ...
      biddingsTable: {
        columns: [...biddingsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'biddingId',           // Column to sort by
  ...
  methods: {
    ...
    async renderBiddingsTable() {
      ...
      let biddingsTableData = [];
      for (let i = 0; i < this.biddings.length; i++) {
        biddingsTableData.push({
          id: i,
          biddingId: this.biddings[i].biddingId,
          year: this.biddings[i].year,
          date: this.biddings[i].date,
          competitionId: this.biddings[i].competitionId,
          biddingId: this.biddings[i].biddingId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-biddings',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    routingToBiddingDetail(id) {
      this.$router.push({ name: 'BiddingDetail', params: { biddingId: id.toString() }})
    },
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForBiddingsChanged', this.searchQuery);
		//this.renderBiddingsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalBiddings = false;

      const currentDate = new Date().getTime();
      this.biddingToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.biddingToAdd);
      console.log(jsonData);
      
      const res = await BiddingService.addBidding(jsonData);
